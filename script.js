window.addEventListener('load', function () {
      ...
      .then((data) => {
        ...
        for (var i = 0; i < data.list.length; i++) {
          ...
          if (data.list[i].dt_txt.indexOf('15:00:00') !== -1) {
            ...
            _appendChild(bodyEl,[titleEl,imgEl,windEl,humidityEl,p1El,p2El]);
  ...
  function searchWeather(searchValue) {
    ...
    fetch(endpoint)
      ...
      .then((data) => {
        ...
        _appendChild(cardBodyEl,[titleEl,tempEl,humidEl,windEl]); 
  ...
  function makeRow(searchValue) {
    ...
    liEl.addEventListener('click', (e) => {
      if (e.target.tagName === 'LI') {
        document.getElementById("search-value").value = e.target.textContent; 
  ...
  const clearList = () => {localStorage.setItem('history', null); location.reload();}
...
/*
  ...
  const _appendChild = (source, children) => {
    children.forEach(x => source.appendChild(x)); 
